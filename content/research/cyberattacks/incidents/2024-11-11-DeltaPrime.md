...
Public analysis diverged on whether that description fully captured the exploit chain. DeltaPrime publicly attributed the incident primarily to reward-claim impersonation. CertiK publicly analyzed it as a chained two-vulnerability exploit that included, but was not limited to, reward-claim abuse: one flaw in debt-swap handling that allowed borrowed assets to be routed to an attacker-controlled contract without normal repayment effects, and another in reward-claim handling that accepted attacker-controlled external input and let collateral be withdrawn as apparent rewards. DeltaPrime's post-mortem reported a total loss of about $4.75 million, while third-party estimates were around $4.85 million.
